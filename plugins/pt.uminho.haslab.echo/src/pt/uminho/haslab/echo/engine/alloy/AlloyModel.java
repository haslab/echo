...
class AlloyModel implements CoreModel {
	...
	final AlloyMetamodel metamodel;
	 ...
	 * @throws EError
	 */
	AlloyModel(EModel emodel, AlloyMetamodel t) throws EError {
		this.emodel = emodel;
		metamodel = t;
		
		try {
			model_sig = new PrimSig(emodel.ID,(PrimSig) t.SIG,Attr.ONE);
		} catch (Err a) {throw new EErrorAlloy (EErrorAlloy.FAIL_CREATE_SIG,a.getMessage(),a,Task.TRANSLATE_MODEL); }
	 ...
	 * @throws EError
	 */
	private PrimSig translateElement(EElement eelement) throws EError {
	 ...
	 * @throws EError
	 */
	private void processValues(List<EValue> values, Field field, PrimSig elementsig) throws EError {
	...
	private void processValue(EValue value, Field field, Sig elementsig)
		...
		if (value instanceof EElement) {
			...
			ExprConstant str = ExprConstant.Op.STRING.make(null, ((EString) value).getValue());
			strings.add(((EString) value).getValue());
	...
	PrimSig setTarget() throws EErrorAlloy {
		isTarget = true;
		try {
			trg_model_sig = new PrimSig(EchoHelper.targetName(emodel.ID), metamodel.SIG, Attr.ONE);
		} catch (Err e) {
			throw new EErrorAlloy("", "Failed to create target sig.", e, Task.CORE_RUN);
		}
