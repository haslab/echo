...
import pt.uminho.haslab.echo.engine.ast.CoreRelation.Mode;
 ...
 */
class AlloyTransformation extends CoreTransformation {
	...
	/** {@inheritDoc} */
	AlloyTransformation(ETransformation transformation,
	...
	protected void createRelation(ERelation rel, EDependency dep, boolean trace)
			throws EError {
		new AlloyRelation(this, dep, rel, trace);
	}

	/** {@inheritDoc} */
	@Override
	protected void initModelParams() throws EErrorAlloy {
	...
	/** {@inheritDoc} */
	@Override
	protected void processConstraint() throws EErrorAlloy {
		...
		try {
			...
			throw new EErrorAlloy(EErrorCore.FAIL_CREATE_FUNC,
	...
	public AlloyFormula getConstraint(List<String> modelIDs) {
		...
		for (String modelID : modelIDs) {
			AlloyModel mdl = (AlloyModel) CoreTranslator.getInstance().getModel(modelID);
	...
	/** {@inheritDoc} */
	@Override
	protected void defineSubRelationField(CoreRelation rel, IFormula def)
			throws EErrorAlloy {
		if (subRelationDefs == null)
		...
		for (AlloyDecl d : ((AlloyRelation) rel).getModelParams())
			decls.add(d.DECL);
		try {
	...
	/** {@inheritDoc} */
	@Override
	protected void addSubRelationField(CoreRelation relation, IExpression exp)
			throws EErrorAlloy {
		...
		for (AlloyDecl d : ((AlloyRelation) relation).getModelParams())
			decls.add(d.DECL);
		try {
	...
	@Override
	public IExpression callAllRelation(ITContext context, IExpression param) throws EErrorAlloy, EErrorUnsupported {
	...
	protected void addTrace() throws EErrorAlloy, EErrorUnsupported {
		if (modelParamDecls.size() > 2) throw new EErrorUnsupported(EErrorUnsupported.MULTIDIRECTIONAL,"Traces between more than 2 models not yet supported.",Task.TRANSLATE_TRANSFORMATION);
		try {
