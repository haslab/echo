 ...
 */
public abstract class CoreRelation {
	...
	public final CoreTransformation transformation;
	...
	public final CoreRelation callerRelation;
	 ...
	 * @throws EError
	 */
	public CoreRelation (ERelation relation, CoreRelation parentRelation) throws EError {
	 ...
	 * @throws EError
	 */
	public CoreRelation (ERelation relation, EDependency dependency, CoreTransformation transformation, boolean trace) throws EError {
	 ...
	 * @throws EError
	 */
	private CoreRelation(ERelation relation, Mode mode,
			EDependency dependency, CoreRelation parentRelation, CoreTransformation transformation)
			throws EError {
		...
		this.context = CoreTranslator.getInstance().newContext();
	 ...
	 */
	private void initVariableLists() throws EError {
		manageModelParams();
...
//		EchoReporter.getInstance().debug("prevars: "+preVar2model);
	 ...
	 * @throws EError
	 */
	private IFormula calculateConstraint() throws EError {
		IFormula formula = CoreTranslator.getInstance().getTrueFormula();
		IFormula sourceFormula = CoreTranslator.getInstance().getTrueFormula();
		IFormula postFormula = CoreTranslator.getInstance().getTrueFormula();
		IFormula whenFormula = CoreTranslator.getInstance().getTrueFormula();
	 ...
	 * @throws EError
	 */
	private IFormula translateCondition(EPredicate predicate) throws EError {
	 ...
	 * @throws EError 
	 */
	protected abstract void manageModelParams() throws EErrorCore, EErrorUnsupported, EErrorParser, EError;
	 ...
	 */
	protected abstract IExpression addNonTopRel(List<? extends EModelDomain> rootVars) throws EError;
	 ...
	 * TODO: Support for n models
	 */
	private void addRelationDef(IExpression field) throws EError {
		if (relation.getDomains().size() > 2) throw new EErrorUnsupported(EErrorUnsupported.MULTIDIRECTIONAL,"Calls between more than 2 models not yet supported.",Task.TRANSLATE_TRANSFORMATION);
	 ...
	 * @throws EError
	 */
	private void addRelationConstraint() throws EError {
	 ...
	 * @throws EError
	 */
	private IExpression addRelationField() throws EError {
		if (relation.getDomains().size() > 2)
			throw new EErrorUnsupported(EErrorUnsupported.MULTIDIRECTIONAL,
					"Calls between more than 2 models not yet supported.",Task.TRANSLATE_TRANSFORMATION);
	 ...
	 * @throws EErrorUnsupported
	 */
	abstract protected IFormula simplify(IFormula formula) throws EErrorUnsupported;

	public abstract void newRelation(ERelation rel) throws EError;
