...
import java.util.*;
 ...
 class AlloyMetamodel extends CoreMetamodel {
	...
	* @throws EError
	*/
	AlloyMetamodel(EMetamodel metamodel) throws EError {
		super(metamodel);
		try {
			//if (EchoOptionsSetup.getInstance().isOperationBased())
			SIG = new PrimSig(metamodel.ID,AlloyTranslator.STATE);
			//else
				//s = new PrimSig(URIUtil.resolveURI(metamodel.eResource()),STATE,Attr.ABSTRACT);
		} catch (Err a) {throw new EErrorAlloy (EErrorAlloy.FAIL_CREATE_SIG,a.getMessage(),a,Task.TRANSLATE_METAMODEL); }
		
		try {
			model_var = SIG.oneOf("s_");
		} catch (Err a) {
			throw new EErrorAlloy(
					EErrorAlloy.FAIL_CREATE_VAR,
	...
	*/
	Field getFieldFromSFeature(EStructuralFeature f) {
		...
		if (fi == null) {
			if (EchoOptionsSetup.getInstance().isOptimize() && f instanceof EReference && ((EReference) f).getEOpposite()!= null) {}
			else EchoReporter.getInstance().warning("Looking for non-existing field: "+f+".",Task.TRANSLATE_METAMODEL);
...
//		EchoReporter.getInstance().debug("Fetching lit "+s.label+" but "+EchoHelper.getFeatureName(s.label)+ " over "+s.parent.label+" but "+EchoHelper.getClassifierName(s.parent.label));
	...
	* @throws EErrorAlloy 
	*/
	protected AlloyFormula getConforms(String modelID) throws EErrorAlloy {
		if(!doneContainments){
        ...
        Func f;
		AlloyModel model = (AlloyModel) CoreTranslator.getInstance().getModel(modelID);
		try {
//			EchoReporter.getInstance().debug("Conforms: "+conformsPred);
			...
			throw new EErrorAlloy(EErrorAlloy.FAIL_CREATE_FUNC,
     ...
     private void makeContainmentConstraint() {
         Expr aux = Sig.NONE.product(Sig.NONE).product(Sig.NONE);
...
//             makeContainmentFact(containers,typeGroups,classifier2sig.get(current));
             for (Field f : mapContainment.get(current))
            	 aux = aux.plus(f);
         }
         try {
        	Expr aux2 = Sig.NONE.product(Sig.NONE);
        	for (Expr x : sig2statefield.values())
        		aux2 = aux2.plus(x);
			Decl d  = (aux2.join(model_var.get())).oneOf("trg_");
            aux = aux.join(model_var.get());
            aux = aux.join(d.get());
            aux = aux.no();
            aux = aux.forOne(d);
            conformsPred = conformsPred.and(aux);
         } catch (Err e) {
     ...
     private void makeContainmentFact(Set<Field> all,Set<Pair<Field,Field>> sameType, PrimSig type){
         try {
             ...
             conformsPred = conformsPred.and(
                     aux.join(d.get()).lone().and(aux.join(d.get()).equal(d.get()).not())
                     .forAll(d));
	...
	* @throws EErrorAlloy 
	*/
     AlloyFormula getGenerate(String modelID) throws EErrorAlloy {
        if(!doneContainments){
            makeContainmentConstraint();
            doneContainments = true;
        }
	...
	* @throws EError
	*/
	protected void processClass(EClass ec) throws EError {
		...
		try {
			...
			throw new EErrorAlloy(EErrorAlloy.FAIL_CREATE_SIG,
	...
	* @throws EErrorUnsupported the attribute type is not supported
	* @throws EErrorAlloy
	*/
		...
		for (EAttribute attr : attributes) {
			...
			try {
				...
				if (attr.getEType().eIsProxy())
					t = (EClassifier) EcoreUtil.resolve(attr.getEType(), metamodel.getEObject());
				else 
					t = attr.getEType();
//				if (t instanceof EEnumImpl)
//					EchoReporter.getInstance().debug(t+""+((EEnumImpl) t).eIsProxy()+((EEnumImpl) t));
				...
				throw new EErrorAlloy(EErrorAlloy.FAIL_CREATE_FIELD,
...
//					return; // ????
			...
			if (cc == null)
				throw new EErrorParser(EErrorParser.METAMODEL,"Failed to find reference '"+reference.getName()+"' type.",
			...
			try {
				...
				if (reference.isContainment()) {
					// should not be needed if containment tree constraints are generated
...
//                    
				...
				throw new EErrorAlloy(EErrorAlloy.FAIL_CREATE_VAR,
	...
	* @throws EErrorTransform the OCL translation failed
	* @throws EErrorAlloy
	* @throws EErrorUnsupported the OCL formulas contains unsupported operators
	* @throws EErrorParser the OCL parser failed
	*/
	protected void processAnnotations(List<EAnnotation> annotations)
		...
		for (EAnnotation annotation : annotations) {
			Decl self = null;
//			EchoReporter.getInstance().debug("Context: "+annotation.eContainer());
...
//				EchoReporter.getInstance().debug("self range: "+statefield.join(model_var.get()));
		...
		for (EOperation operation : eoperations) {
			...
			try {
				...
				for (EParameter p : operation.getEParameters()) {
					PrimSig type = AlloyTranslator.getInstance()
			...
			for (EAnnotation ea : operation.getEAnnotations()) {
				if (ea.getSource().equals(
					...
					for (String sExpr : ea.getDetails().values()) {
						...
						"Echo/@modifies")) {
					IFormula form = context.createFrameCondition(metamodel.ID, ea.getDetails().values());
					oclalloy = oclalloy.and(((AlloyFormula) form).FORMULA);
				}
			}
			try {
	...
	* @throws EErrorAlloy
	*/
	protected void processEnums(List<EEnum> enums) throws EErrorAlloy {
	...
	* @throws EErrorAlloy
	*/
	private void processHeritage(PrimSig s) throws EErrorAlloy {
		...
		try {
			...
			throw new EErrorAlloy(EErrorAlloy.FAIL_GET_CHILDREN,
	...
	* @throws EErrorAlloy
	*/
	Func getDeltaSetFunc() throws EErrorAlloy {
		Decl dm, dn;
	...
	* @throws EErrorAlloy
	*/
	Func getDeltaRelFunc() throws EErrorAlloy {
		Decl dm, dn;
		List<Decl> ds = new ArrayList<Decl>();
		try {
			dm = SIG.oneOf("m_");
	...
	* @throws EErrorAlloy
	*/
	void createOrder() throws EErrorAlloy {
		PrimSig ord;
		Field next, first;
		try {
		...
		try {
			...
			throw new EErrorAlloy(EErrorAlloy.FAIL_CREATE_FACT,
