...
public class EchoReporter {
	...
	private Map<Task,String> initmessage = new HashMap<Task,String>();
	private Map<Task,String> endmessage = new HashMap<Task,String>();
	...
	public void start(Task qualifier,String message) {
		...
		initmessage.put(qualifier, message);
	}

	public long getInitTime(Task qualifier) {
		return inittime.get(qualifier);
	}

	public long getEndTime(Task qualifier) {
		return endtime.get(qualifier);
	}

	public String getInitMessage(Task qualifier) {
		return initmessage.get(qualifier);
	}

	public void result (Task qualifier, String message, boolean result) {
		endtime.put(qualifier, System.currentTimeMillis() - getInitTime(qualifier));
		endmessage.put(qualifier, message);
	}
